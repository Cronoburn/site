...
$app->match('/login', function(Request $request) use ($app) {
    $form = $app['form.factory']->createBuilder('form')
        ...
        ->getForm();
$app['session']->getFlashBag()->add('error', 'This Site is under HEAVY construction!<br />Proceed with caution!');
...
$app->match('/resume', function() use ($app){
    return $app['twig']->render(
        ...
        array(
            ...
            'city' => 'MOUNT VERNON',
            'phone' => '(812)575-7072',
			'records' =>  $app['db']->fetchAll('SELECT * FROM resume')
        )
    );
})->bind('resume');
...
$app->match('/blog/{ID}',function (Request $request) use($app) {
    $app['twig']->render('blog.html.twig',array(
        'records'=> $app['db']->fetchAll('SELECT * FROM blog_posts LIMIT 5')
    ));
})->method('GET');
